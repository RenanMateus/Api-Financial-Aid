...
function accountRouter() {
    ...
    router.post('/account', auth, async (req, res) => {
        try {
            ...
            if (await Account.findOne({ name: name, _owner: _user }) || (await Account.findOne({ bank: bank, agency: agency, accountNumber: accountNumber, _owner: _user }) && (type == 'Conta Corrente' || type == 'Conta Poupanca')))
    ...
    router.put('/account/:id', auth, async (req, res) => {
        try {
            ...
            if (registered && registered._id != req.params.id && (type == 'Conta Corrente' || type == 'Conta Poupanca'))
                return res.status(409).send({ error: { message: 'Conta já cadastrada!' } });
    ...
    const storage = multer.diskStorage({
        destination: function (req, file, cb) {
            cb(null, path.resolve("uploads", "transactions"));
        },
